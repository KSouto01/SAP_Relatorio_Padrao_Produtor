# VERSÃO: 12.9 - Correção do Colapso Visual (Hierarquia Flexbox Estrita)
import dash
...
app.title = "Relatório Produtor v12.9"
server = app.server
...
# CONFIGURAÇÃO DE LAYOUT: O segredo é d-flex e flex-column em todos os níveis
app.layout = dbc.Container([
    dcc.Store(id="store-dados"),
    dcc.Store(id="store-lista-fornecedores"),

    # 1. CABEÇALHO (Fixo - flex-shrink-0 impede de encolher)
    dbc.Navbar(
        dbc.Container([
            dbc.Row([
    ...
    dbc.Card([
        dbc.CardBody([
            ...
            dbc.Row([
                ...
                dbc.Col([dbc.Label("Material", className="small mb-0 fw-bold text-primary"), dcc.Dropdown(id="dd-material", placeholder="Todos", clearable=True, className="small-dropdown-sm")], width=3),
    ...
    # 4. TABELA (Área Flexível - Ocupa o resto da tela)
    html.Div(
        className="flex-grow-1 mx-2 mb-0 border rounded overflow-hidden bg-white d-flex flex-column",
        # minHeight: 0 é CRUCIAL para o flexbox entender que pode encolher o conteúdo se necessário para caber na tela
        style={"minHeight": "0"}, 
        children=[
            dcc.Loading(
                id="loading-wrapper",
                color="#EF6100",
                # O Loading também precisa ser flex para esticar o filho (area-tabela)
                parent_style={"flex": "1", "display": "flex", "flexDirection": "column", "overflow": "hidden"},
                children=[
                    html.Div(
                        id="area-tabela", 
                        # A div que segura a tabela também precisa ser flex
...
@app.callback(Output("store-dados", "data"), Output("dd-material", "options"), Output("dd-material", "value"), Input("btn-carregar", "n_clicks"), State("dt-inicio", "value"), State("dt-fim", "value"), State("dd-codigo", "value"), prevent_initial_call=True)
def buscar_dados_sap(n, start, end, parceiro_id):
    ...
    return df.to_dict('records'), [{'label': m, 'value': m} for m in materiais], None
...
def atualizar_tabela_totais(data, material_selecionado):
    ...
    if material_selecionado:
        ...
        if df.empty: return dbc.Alert("Sem dados para este material.", color="warning", className="m-5"), []
    ...
    tabela = dash_table.DataTable(
        ...
        # style_table agora sem position absolute, confiando no Flexbox pai
